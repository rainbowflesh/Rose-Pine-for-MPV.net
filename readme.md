<p align="center">
    <img src="https://github.com/rose-pine/rose-pine-theme/raw/main/assets/icon.png" width="80" />
    <h2 align="center">Rosé Pine for MPV.net Media Player</h2>
</p>

<p align="center">All natural pine, faux fur and a bit of soho vibes for the classy minimalist</p>

<p align="center">
    <a href="https://github.com/rose-pine/rose-pine-theme">
        <img src="https://img.shields.io/badge/community-rosé%20pine-26233a?labelColor=191724&logo=data:image/svg+xml;base64,&style=for-the-badge" />
    </a>
</p>

## Usage

1. Get [mpv.net](https://github.com/mpvnet-player/mpv.net)
2. Put [theme.conf](./theme.conf) to `<conf folder>\theme.conf`, mostly in `~\AppData\Roaming\mpv.net\`.
3. Under `settings/Appearance`, set `rose-pine` or `rose-pine-moon` to `dark-theme`, `rose-pine-dawm` to `light-theme`
4. Restart program, done!

## Gallery

### Base
![rose-pine](./base.png)

### Moon
![rose-pine-moon](./moon.png)

### Dawn
![rose-pine-dawn](./dawn.png)

## Thanks to

- [Me, it's me!](https://github.com/rainbowflesh)
- [And Rose Pine!](https://github.com/rose-pine/rose-pine-theme) Best palette ever!

## Contributing

Modify `theme.conf` then PR ;)
